...
export const LoanRequest = ({
  ...
  const handleHeaderToggle = () => {
    ...
    if (newHeaderState) {
      const allloan_uuids = laon_table_data_all.map(
        (item: LoanData) => item.loan_uuid
      );
  ...
  const renderRow = useCallback(
    (item: LoanData, index: number) => (
      <>
        ...
        <td className="truncate max-w-[75px] py-4 px-6">{item.credit_score}</td>
        ...
        <td
          className="truncate max-w-[154px] py-4 px-6 loan-request-quick-actions"
    ...
    [
      isModalOpenApproveRequest,
      isModalOpenDeclineRequest,
      isModalOpen,
      selectedIds,
      handleToggle,
    ]
